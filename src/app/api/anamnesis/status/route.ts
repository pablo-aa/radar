// GET /api/anamnesis/status?run_id=<uuid>
// or no query param: returns the latest run for the authenticated user.
//
// Response shapes:
//   200 { run_id, status: "running"|"done"|"error", started_at, finished_at, elapsed_seconds }
...
import type { AnamnesisRun } from "@/lib/supabase/types";
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  let row: AnamnesisRun | null = null;
  ...
  if (runIdParam) {
    ...
    if (error) {
      console.error("[api/anamnesis/status] db error", error);
    ...
    if ((data as AnamnesisRun).user_id !== userId) {
      return NextResponse.json({ error: "not_found" }, { status: 404 });
    }

    row = data as AnamnesisRun;
  } else {
